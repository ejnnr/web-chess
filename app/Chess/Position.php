...
class Position
{
     ...
     */
    public function __construct($position = 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1')
    {
    ...
    public function getArray()
    {
        ...
        foreach ($this->board as $index => $square) {
            if ($index % 8 == 0) { // start of a new rank
                $ret[] = []; // add a new rank (array) to the return value
            }
    ...
    public function loadFEN($fen)
    {
        ...
        $matches = []; // used to store the match result of the RegExp
        ...
        foreach ($ranks as $rank) {
            // go through all of the ranks (seperated by '/' in fen)

            $i_file = 0;

            while (!empty($rank)) {
                // since every square that was parsed is removed from $rank, this expression can be used to check if we are done
                ...
                if (is_numeric(substr($rank, 0, 1))) {
                    ...
                    if ($new == 0) {
                        // if the new number is zero, set it to ''
        ...
        if (strpos($sections[2], 'K') !== false) {
            // '!== false' is necessary. See http://php.net/manual/en/function.strpos.php for further information
     ...
     *
     * @param bool  $side  The side to be checked (true for white, false for black)
     ...
     */
    public function isLegalMove(Move $move)
    {
     ...
     *
     * @param int $departure   the square of departure
     * @param int $destination the square of destination
     *
     ...
     * 
     * @param int   $departure   the square currently occupied by the piece
     * @param int   $destination the square that is (not) attackes
     * @param array $board       the board to use for the checks (equal to $this->board if left out)
     *
     * @return bool true if the piece attacks the square, otherwise false
     */
     ...
     *
     * @return bool true if it is attacked, otherwise false
     */
    ...
    public function parseSAN($move)
    {
        ...
        $annotationMove = preg_replace(['/^!$/', '/^\?$/', '/^!!$/', '/^\?\?$/', '/^!\?$/', '/^\?!$/'], ['\$1', '\$2', '\$3', '\$4', '\$5', '\$6'], $matches['annotationMove']);
